...
import { loadOrCreateConfig, loadConfig, saveConfig, atomicConfigUpdate, getConfigPath, getCrashLogPath, loadState, saveState } from './config.ts';
import { installCrashHandlers } from './crash-log.ts';
import { AccountManager } from './account-manager.ts';
import { createProxyServer } from './server.ts';
import { importCredentials, loginOAuth, fetchProfile, refreshAccessToken, isTokenExpiringSoon } from './oauth.ts';
import type { ImportedCredentials, ProfileResult, RefreshedTokens } from './oauth.ts';
import { sameIdentity, orgKey, matchAccounts, findUpsertTarget, orgLabel, withOrgSuffixes } from './identity.ts';
import type { Account, AccountConfig, Config, Dict, ProxyClientConfig, RequestInfo, RouteConfig, ServerHooks } from './types.ts';
import { resolveAccounts } from './resolve-accounts.ts';
import * as alias from './alias.ts';
import { ensureCerts } from './mitm.ts';
import { Prober } from './prober.ts';
import { Warmer } from './warmer.ts';
import { TUI } from './tui.ts';
import { RemoteControl, createAttachSession } from './tui-remote.ts';
import { SxManager } from './sx.ts';
import { autoUpdate, checkForUpdate, currentVersion, runUpdate, installKind, PKG_NAME } from './updater.ts';
import { renderStatus } from './status-renderer.ts';
import { buildClaudeEnvLines, encodePinComponent } from './claude-env.ts';
import { serviceKind, installService, uninstallService, serviceStatus, renderService, logPath } from './service.ts';
import { formatTerminalTitle, titleSequence, TITLE_STACK_PUSH, TITLE_STACK_POP } from './terminal-title.ts';
import { getUpstreamProxy, describeProxy } from './upstream-proxy.ts';
import { clientById, generateClientSecret, hashClientSecret, resolvePrincipal, validClientId } from './client-auth.ts';
import { AuditLog, auditHooks } from './audit-log.ts';
...
type AccountWithProfile = { account: AccountConfig; profile: ProfileResult | null };

/** A credential read back from a disk config entry (oauth tokens or an API key). */
interface DiskCredential {
  accessToken?: string;
...
function resolveCommand(name?: string): [() => void | Promise<void>, boolean] {
  if (name && Object.hasOwn(COMMANDS, name)) return [COMMANDS[name], !NO_EXIT.has(name)];
...
async function serverCommand() {
  ...
  const onListenError = (err: NodeJS.ErrnoException) => handleServerListenError(err, port);
...
function saveQuotaState(accountManager: AccountManager): Promise<void> {
  return saveState({ quota: accountManager.exportQuotaState() })
    .catch((err: any) => console.error(`[Jaynshare] Failed to save quota state: ${err.message}`));
}

/** Returns a stop() that also flushes the state one last time. */
function persistQuotaPeriodically(accountManager: AccountManager, everyMs = 60_000): () => Promise<void> {
...
function serverHooks(
  { tui, activityLogPath }: { tui: TUI | null; activityLogPath: string | null },
  { config, background, reloadAccounts }: { config: Config; background: { prober: Prober; warmer: Warmer }; reloadAccounts: () => Promise<number> },
): ServerHooks {
  const startedAt = Date.now();
  let hooks: ServerHooks = {};
  if (tui) {
    hooks = {
      onRequestStart: (id: number, info: RequestInfo) => tui.onRequestStart(id, info),
      onRequestModel: (id: number, info: RequestInfo) => tui.onRequestModel(id, info),
      onRequestRouted: (id: number, info: RequestInfo) => tui.onRequestRouted(id, info),
      onRequestEnd: (id: number, info: RequestInfo) => tui.onRequestEnd(id, info),
    };
  ...
  if (auditLog?.path) hooks = auditHooks(new AuditLog({ path: auditLog.path, maxBytes: auditLog.maxBytes, keepFiles: auditLog.keepFiles }), hooks);
...
function makeShutdown({ server, tui, background, stop }: {
  server: http.Server; tui: TUI | null; background: { prober: Prober; warmer: Warmer }; stop: (() => unknown)[];
}) {
...
async function resolveServerAccounts(config: Config): Promise<AccountConfig[]> {
...
async function restoreQuota(accountManager: AccountManager): Promise<void> {
  const savedState = await loadState().catch((err: any) => {
...
function persistRefreshedTokens(accountManager: AccountManager, config: Config): void {
  accountManager.onTokenRefresh((idx: number, newTokens: { accessToken: string; refreshToken?: string; expiresAt: number }) => {
    ...
    }).catch((err: any) => console.error(`[Jaynshare] Failed to save refreshed token: ${err.message}`));
  });
}

async function createSxManager(config: Config): Promise<SxManager> {
...
function makeReloadAccounts({ config, accountManager, sx, prober, warmer }: {
  config: Config; accountManager: AccountManager; sx: SxManager; prober: Prober; warmer: Warmer;
}): () => Promise<number> {
  return async () => {
...
function writeRuntimeConfig(diskConfig: Config, config: Config, accountManager: AccountManager): void {
  // Live tokens win; disk-only fields (importFrom) survive.
  diskConfig.accounts = config.accounts.map((a: AccountConfig, i: number) => {
...
function activityLogHooks(activityLogPath: string): ServerHooks {
  const aStream = createWriteStream(activityLogPath, { flags: 'a' });
  aStream.on('error', (err: any) => process.stderr.write(`[Jaynshare] activity log error: ${err.message}\n`));
  const ts = () => new Date().toLocaleTimeString('en-US', { hour12: false });
  const writeActivity = (msg: string): void => {
    aStream.write(`${ts()}  ${msg.replace(/^\[Jaynshare\]\s*/, '')}\n`);
  };

  const inFlight = new Map<number, Dict>();
  const hooks: ServerHooks = {
    onRequestStart: (id: number, info: RequestInfo) => inFlight.set(id, { ...info, started: Date.now() }),
...
function printStartupBanner({ bindHost, port, accounts, threshold, config }: {
  bindHost: string; port: number; accounts: AccountConfig[]; threshold: number; config: Config;
}): void {
  const sep = '='.repeat(60);
  console.log('');
  console.log(sep);
  ...
  console.log('');
  accounts.forEach((a: AccountConfig, i: number) => {
...
async function loginCommand(): Promise<void> {
  ...
  console.log('');
  const choice = await new Promise<string>(resolve => rl.question('Choice [1]: ', resolve));
  rl.close();
...
async function loginApiCommand(): Promise<void> {
  const config = await loadOrCreateConfig();
  let name = argValue('--name');

  const rl = createInterface({ input: process.stdin, output: process.stderr });
  const apiKey = await new Promise<string>(resolve => rl.question('Anthropic API key: ', resolve));
  rl.close();
...
async function loginOAuthCommand(): Promise<void> {
  const config = await loadOrCreateConfig();
  let name = argValue('--name');

  console.log('Starting OAuth login...');
  let creds: RefreshedTokens;
  try {
...
async function envCommand(): Promise<void> {
...
async function printEnvNotes(config: Config, { port, useMitm, account }: { port: number; useMitm: boolean; account: string }): Promise<void> {
...
function splitRunArgs(rest: string[]): { flags: string[]; claudeArgs: string[] } {
...
async function withProxyEnv(env: NodeJS.ProcessEnv, { config, account, useMitm }: {
  config: Config; account: string; useMitm: boolean;
...
function refuseOrFallBack(port: number, autoFallback: boolean): void {
  if (autoFallback) {
...
function withHoldTimeout(env: NodeJS.ProcessEnv, holdSeconds?: number): NodeJS.ProcessEnv {
...
async function attachCommand(): Promise<void> {
  const config = await loadOrCreateConfig();
  const port = config.proxy.port || 3456;
  ...
  const control = new RemoteControl({ port, host, apiKey: config.proxy.apiKey || null });
  let first;
  try {
...
async function listServerAccounts({ port, headers }: { port: number; headers: Record<string, string> }): Promise<void> {
  const res = await fetch(`http://localhost:${port}/jaynshare/status`, { headers });
  const data = (res.ok ? await res.json().catch(() => null) : null) as Dict | null;
...
async function requestSwitch({ port, headers, name }: {
  port: number; headers: Record<string, string>; name: string;
}): Promise<void> {
...
async function refreshExpiringTokens(accounts: AccountConfig[]): Promise<boolean> {
  let changed = false;
  await Promise.all(accounts.map(async (a: AccountConfig) => {
...
async function withProfiles(accounts: AccountConfig[]): Promise<AccountWithProfile[]> {
  const profiles = await Promise.all(accounts.map((a: AccountConfig) =>
    a.type === 'oauth' && a.accessToken ? fetchProfile(a.accessToken) : null
  ));
  return accounts.map((account: AccountConfig, i: number) => ({ account, profile: profiles[i] }));
}

/** Copies identity the profile knows onto the config entry; returns whether anything changed. */
function backfillIdentity(entries: AccountWithProfile[]): boolean {
  let touched = false;
...
function withoutDuplicateIdentities(entries: AccountWithProfile[]): AccountWithProfile[] {
  const seen = new Set();
  const kept = [];
...
function nameFromEmail(entries: AccountWithProfile[]): boolean {
...
function printAccount({ account, profile }: AccountWithProfile, { position, verbose }: { position: number; verbose: boolean }): void {
...
async function apiCommand(): Promise<void> {
  const config = await loadOrCreateConfig();
  const path = args[1];
...
async function apiAccount(config: Config): Promise<AccountConfig> {
  const accounts = await resolveAccounts(config);
...
function apiRequestInit(account: AccountConfig): RequestInit {
...
async function printApiResponse(res: Response): Promise<void> {
...
async function serviceCommand(): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'print': {
      const unit = renderService({ configPath });
...
async function warmupCommand(): Promise<void> {
  const config = await loadOrCreateConfig();
  const arg = args[1];
...
function readIntervalArg(arg: string | undefined, { usage, minSeconds, tooShort }: {
  usage: string; minSeconds: number; tooShort: string;
}): number {
...
function resolveAccount(accounts: AccountConfig[], query: string, orgFilter?: string | null): AccountConfig | null {
  const matches = matchAccounts(accounts, query, orgFilter ?? undefined);
...
const ROUTE_SUBCOMMANDS: Record<string, (config: Config) => void | Promise<void>> = {
  list: listRoutes,
  add: addRoute,
  rm: removeRoute,
  remove: removeRoute,
...
async function routeCommand(): Promise<void> {
...
function listRoutes(config: Config): void {
  config.routes ||= [];
...
async function addRoute(config: Config): Promise<void> {
  config.routes ||= [];
  const route = readRouteFlags(config);
  const existingIndex = config.routes.findIndex((r: RouteConfig) => r.name === route.name);
...
function readRouteFlags(config: Config): RouteConfig {
  ...
  const route: RouteConfig = { name, match };
...
async function removeRoute(config: Config): Promise<void> {
  const name = args[2];
  config.routes ||= [];
  const before = config.routes.length;
  config.routes = config.routes.filter((r: RouteConfig) => r.name !== name);
...
async function priorityCommand(): Promise<void> {
  const config = await loadOrCreateConfig();
  const name = args[1];
  ...
  const priorities = config.accounts.map((a: AccountConfig) => a.priority || 0);
  let priority;
...
const DISABLE: RotationChange = { verb: 'disable', done: 'Disabled', apply: (a: AccountConfig) => { a.disabled = true; } };
const ENABLE: RotationChange = { verb: 'enable', done: 'Enabled', apply: (a: AccountConfig) => { delete a.disabled; } };

interface RotationChange {
  verb: string;
  done: string;
  apply: (a: AccountConfig) => void;
}

async function setAccountRotation({ verb, done, apply }: RotationChange): Promise<void> {
  const config = await loadOrCreateConfig();
  const name = args[1];
...
interface ClientStateChange {
  ...
  apply: (client: ProxyClientConfig, config: Config) => void;
}

const CLIENT_STATE_CHANGES: Record<string, ClientStateChange> = {
  disable: { done: 'Disabled', apply: (client: ProxyClientConfig) => { client.disabled = true; } },
  enable: { done: 'Enabled', apply: (client: ProxyClientConfig) => { client.disabled = false; } },
  revoke: { done: 'Revoked', apply: (client: ProxyClientConfig, config: Config) => { config.proxy.clients = (config.proxy.clients || []).filter((c: ProxyClientConfig) => c.id !== client.id); } },
};

async function clientCommand(): Promise<void> {
...
async function printClientEnv(): Promise<void> {
  const id = args[2];
...
async function changeClientState(sub: string): Promise<void> {
...
async function rotateAdminCredential(): Promise<void> {
...
function requireClient(config: Config, id: string): ProxyClientConfig {
...
function announceSecret(secret: string, notice: string): void {
  process.stdout.write(`${secret}\n`);
  process.stderr.write(`[Jaynshare] ${notice} This secret will not be shown again.\n`);
}

function clientCommandError(err: any): never {
  console.error(`[Jaynshare] ${err.message}`);
  process.exit(1);
}

function clientUsage(message?: string | null): never {
...
async function upsertOAuthAccount(config: Config, { name, creds, source = 'unknown' }: {
  name: string | null; creds: ImportedCredentials; source?: string;
}): Promise<void> {
...
function derivedAccountName(accounts: AccountConfig[], profile: ProfileResult | null): string {
  if (profile?.email) {
    const tier = profile.hasClaudeMax ? 'Max' : profile.hasClaudePro ? 'Pro' : null;
    if (tier) console.log(`Detected Claude ${tier} account: ${profile.email}`);
    return profile.email;
  }
  return `account-${accounts.filter((a: AccountConfig) => a.name.startsWith('account-')).length + 1}`;
}

function oauthEntry({ name, creds, source, profile }: {
  name: string; creds: ImportedCredentials; source: string; profile: ProfileResult | null;
}): AccountConfig {
  return {
    name,
    type: 'oauth',
...
function findConfigAccount(diskConfig: Config, account: AccountConfig): number {
  return diskConfig.accounts.findIndex((a: AccountConfig) => sameIdentity(a, account));
}

/** Returns the number of accounts added. */
async function syncAccountsFromDisk(diskConfig: Config, memConfig: Config, accountManager: AccountManager): Promise<number> {
  let added = 0;
...
function oneToOneClaim(accountManager: AccountManager): (diskAccount: AccountConfig) => number {
  const claimed = new Set<number>();
  return (diskAccount: AccountConfig): number => {
...
function applyDiskFields(account: Account, diskAccount: AccountConfig, accountManager: AccountManager): void {
...
async function readDiskCredential(diskAccount: AccountConfig): Promise<DiskCredential | null> {
...
function applyCredential(account: Account, freshCredential: DiskCredential, accountManager: AccountManager): void {
  if (freshCredential.accessToken) {
    ...
    if (changed && !diskIsStaler) {
      accountManager.updateAccountTokens(account.index, {
        accessToken: freshCredential.accessToken,
        refreshToken: freshCredential.refreshToken ?? undefined,
        expiresAt: freshCredential.expiresAt ?? null,
      });
...
function isLocalAccountPin(url: string | undefined, port: number): boolean {
  if (!url) return false;
  let u;
...
function upstreamHost(config: Config): string {
...
function startTerminalTitleUpdater(accountManager: AccountManager): () => void {
...
async function notifyRunningServer(config: Config): Promise<void> {
...
function isProxyUp(port: number, timeout = 600): Promise<boolean> {
  return new Promise(resolve => {
    const socket = net.connect({ host: '127.0.0.1', port });
    const done = (up: boolean): void => { socket.destroy(); resolve(up); };
...
function handleServerListenError(err: NodeJS.ErrnoException, port: number): never {
