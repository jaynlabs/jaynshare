import { importCredentials } from './oauth.ts';
import type { Config } from './types.ts';

// Config accounts with credentials resolved; entries without a usable credential are dropped.
export async function resolveAccounts(config: Pick<Config, 'accounts'>): Promise<any[]> {
  const accounts: any[] = [];
    ...
    if (acct.type === 'oauth') {
      if (acct.importFrom) {
        try {
          ...
          console.error(`Failed to import "${acct.name}": ${(err as Error).message}`);
        }
