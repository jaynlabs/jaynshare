import { refreshAccessToken, isTokenExpiringSoon, isTokenExpired } from './oauth.ts';
import type { RefreshedTokens } from './oauth.ts';
import { sameIdentity } from './identity.ts';
import { weeklyBucketForModel, modelGlobMatches } from './model.ts';
import { SessionTracker } from './session-tracker.ts';
import type { Account, AccountConfig, Dict, Quota, RouteAccountView, RouteView, TuiAccountManagerLike } from './types.ts';

export { isFableModel, parseRequestModel, parseAdvisorModel } from './model.ts';
...
function makeAccount(acct: AccountConfig, index: number): Account {
  return {
    index,
    name: acct.name,
    type: acct.type,
...
function unifiedLimits(headers: Dict): Partial<Quota> {
...
function legacyLimits(headers: Dict): Partial<Quota> {
  const patch: Record<string, any> = {};
  const counters = {
...
function usagePercent(quota: Quota): string {
...
function modelMatches(declared: string, model: string): boolean {
  return declared === model || declared.replace(/\[\d+m\]$/, '') === model;
}

function sampleModelFor(route: { match: string[] }): string {
...
export interface AccountManagerOptions {
  refreshFn?: (refreshToken: string, endpoint?: string) => Promise<RefreshedTokens>;
  throttleProbeFloorMs?: number;
...
export class AccountManager implements TuiAccountManagerLike {
  _forcedRefreshFloorMs: number;
  _refreshFn: (refreshToken: string, endpoint?: string) => Promise<RefreshedTokens>;
  accounts: Account[];
  currentIndex = 0;
  sessionTracker: SessionTracker;
  distributeSessions: boolean;
  routePins = new Map<string, number>(); // routeName → account index; runtime only
  switchThreshold: number;
  routes: RouteDef[] = [];
  ramp: { enabled: boolean; startConc: number; stepConc: number; stepMs: number; windowMs: number; pollMs: number };
  probeIntervalMs = 60_000;
  _nextProbeAt = 0;
  _advisorDegradeLogAt = 0;
  _onTokenRefresh?: (accountIndex: number, tokens: { accessToken: string; refreshToken?: string; expiresAt: number }) => void;
  throttleProbeFloorMs: number;

  constructor(accounts: AccountConfig[], switchThreshold = 0.98, { refreshFn = refreshAccessToken, throttleProbeFloorMs, forcedRefreshFloorMs = FORCED_REFRESH_FLOOR_MS, routes, ramp, distributeSessions = false, sessionTracker }: AccountManagerOptions = {}) {
  ...
  _beginRamp(account: Account | null | undefined): void {
    if (account && this.ramp.enabled) account.rampStartedAt = Date.now();
  }

  _rampCap(account: Account, now: number = Date.now()): number {
   ...
   */
  getActiveAccount(query: SelectionQuery = {}): Account | null {
  ...
  _preferredAccount({ exclude, model, advisorModel, preferredIndex }: SelectionQuery): Account | null {
  ...
  _logAdvisorDegraded(advisorModel: string): void {
   ...
   */
  _select({ exclude, model, advisorModel }: SelectionQuery): Account | null {
  ...
  _selectForSession({ sessionId, exclude, model, advisorModel }: SelectionQuery): Account | null {
  ...
  _pickLeastLoaded(exclude: Set<number> | null = null, model: string | null = null, advisorModel: string | null = null): Account | null {
    const now = Date.now();
    let best = null;
  ...
  recordSession(sessionId: string | null, accountIndex: number): void {
  ...
  sessionStats(): { known: number; active: number; perAccount: Record<number, number> } {
    return this.sessionTracker.stats();
  }

  /** Reads without extending the session's expiry. */
  sessionAssignment(sessionId: string): { account: string; lastRoutedAt: string } | null {
  ...
  preferenceEligibility(accountIndex: number, model: string | null = null, advisorModel: string | null = null): { eligible: boolean; reason?: string } {
  ...
  _ineligibilityReason(account: Account): string {
  ...
  async getActiveAccountFresh(query: SelectionQuery = {}): Promise<Account | null> {
  ...
  _isProbeable(account: Account | null | undefined): boolean {
  ...
  _maxUtilization(account: Account, model: string | null = null): number {
  ...
  _governingWeekly(account: Account, model: string | null): number | null {
    const quota = account.quota;
  ...
  _governingWeeklyReset(account: Account, model: string | null): number | null {
    const quota = account.quota;
    const key = this._weeklyBucketFor(model);
  ...
  _modelWeeklyExhausted(account: Account, model: string | null): boolean {
    const quota = account.quota;
    const key = this._weeklyBucketFor(model);
  ...
  _selectProbe(exclude: Set<number> | null = null, model: string | null = null): Account | null {
    const now = Date.now();
  ...
  _isAvailable(account: Account | null | undefined, model?: string | null, advisorModel?: string | null): boolean {
    if (!account) return false;
    if (account.disabled) return false;
  ...
  _preemptedBy(account: Account, { model = null, advisorModel = null, exclude = null }: { model?: string | null; advisorModel?: string | null; exclude?: Set<number> | null } = {}): Account | null {
  ...
  eligibility(accountIndex: number, model: string | null = null, advisorModel: string | null = null): { eligible: boolean; reason?: string } {
    const account = this.accounts[accountIndex];
    if (!account) return { eligible: false, reason: 'no such account' };
  ...
  setRoutes(routes: any): void {
    this.routes = (Array.isArray(routes) ? routes : []).map((r: any, i: number): RouteDef => ({
      name: r.name || `route-${i + 1}`,
      match: (Array.isArray(r.match) ? r.match : [r.match]).filter((g: unknown) => typeof g === 'string' && g),
  ...
  _routeAllows(account: Account, model?: string | null): boolean {
    const route = this._routeForModel(model);
    if (route && route.accounts.length) {
      return route.accounts.includes(account.name) || route.accounts.includes(String(account.index));
    }
    return this._accountOwnsModel(account, model as string);
  }

  /** @deprecated Use `routes` with an `accounts` list instead. */
  _accountOwnsModel(account: Account, model: string): boolean {
    const owns = (a: Account) => !!a.models?.some(m => modelMatches(m, model));
  ...
  _routeTarget(model: string): string | null {
    const idx = this.previewRouteIndex(model);
    return idx == null ? null : (this.accounts[idx]?.name ?? null);
  }

  _pinnedName(routeName: string): string | null {
    const idx = this.routePins.get(routeName);
    return idx == null ? null : (this.accounts[idx]?.name ?? null);
  }

  _routeAccountsView(route: RouteDef): RouteAccountView[] {
    const sample = sampleModelFor(route);
    const inRoute = (a: Account) => !route.accounts.length
  ...
  _routeSample(routeName: string): string | null {
  ...
  clearRoutePin(routeName: string): void { this.routePins.delete(routeName); }
  ...
  _pinnedAccountForModel(model?: string | null, advisorModel?: string | null): Account | null {
    return this._pinnedFor(model)
      || (advisorModel ? this._pinnedFor(advisorModel) : null);
  }

  _pinnedFor(model?: string | null): Account | null {
  ...
  _clearExpiredQuotas(account: Account): { changed: boolean; session: boolean } {
    const quota = account.quota;
    const now = Date.now();
    let changed = false;
    let session = false;
  ...
  refreshExpiredQuotas(): boolean {
    let changed = false;
    const sessionReset: Account[] = [];
    for (const account of this.accounts) {
  ...
  _switchOnSessionReset(candidates: Account[]): void {
    const current = this.accounts[this.currentIndex];
  ...
  _isNearQuota(account: Account, model: string | null = null): boolean {
    const quota = account.quota;
  ...
  _pickBestAvailable(exclude: Set<number> | null = null, model: string | null = null, advisorModel: string | null = null): Account | null {
  ...
  selectActiveAccount(): Account | null {
    this.refreshExpiredQuotas();
  ...
  _selectNext(exclude: Set<number> | null = null, model: string | null = null, advisorModel: string | null = null): Account | null {
  ...
  updateQuota(accountIndex: number, headers: Dict): void {
    const account = this.accounts[accountIndex];
    if (!account) return;
  ...
  _resolveProbe(account: Account): void {
  ...
  updateUsage(accountIndex: number, inputTokens: number, outputTokens: number): void {
    const account = this.accounts[accountIndex];
    if (!account) return;
  ...
  setDisabled(accountIndex: number, disabled: boolean): void {
    const account = this.accounts[accountIndex];
    if (!account) return;
  ...
  applyUsageData(accountIndex: number, usage: any): void {
    const account = this.accounts[accountIndex];
  ...
  markRateLimited(accountIndex: number, retryAfterSeconds: number): void {
    const account = this.accounts[accountIndex];
    if (!account) return;
  ...
  clearRateLimited(accountIndex: number): void {
    const account = this.accounts[accountIndex];
  ...
  async ensureTokenFresh(accountIndex: number): Promise<void> {
    const account = this.accounts[accountIndex];
  ...
  async refreshTokenAfterRejection(accountIndex: number): Promise<void> {
    const account = this.accounts[accountIndex];
    if (!this._refreshable(account)) return;
  ...
  _refreshable(account: Account | null | undefined): boolean {
    return !!(account && account.type === 'oauth' && account.refreshToken);
  }

  _refreshToken(accountIndex: number, account: Account): Promise<void> {
    if (account._refreshPromise) return account._refreshPromise as Promise<void>;
    ...
    account._refreshPromise = (async () => {
      ...
      try {
        ...
        console.error(`[Jaynshare] Token refresh failed for "${account.name}": ${e.message}`);
        // Only a rejected refresh token sidelines the account; a transient failure retries next request.
        const isAuthRejection = e.status === 400 || e.status === 401 || e.status === 403;
        if (isAuthRejection) {
  ...
  onTokenRefresh(callback: (accountIndex: number, tokens: { accessToken: string; refreshToken?: string; expiresAt: number }) => void): void {
    this._onTokenRefresh = callback;
  }

  updateAccountTokens(accountIndex: number, { accessToken, refreshToken, expiresAt }: { accessToken: string; refreshToken?: string | null; expiresAt: number | null }): void {
    const account = this.accounts[accountIndex];
  ...
  addAccount(acctData: AccountConfig): number {
  ...
  restoreQuotaState(saved: any): void {
    if (!Array.isArray(saved)) return;
    for (const account of this.accounts) {
      const match = saved.find((s: any) => sameIdentity(s, account));
  ...
  getStatus() {
    ...
    return {
      ...
      accounts: this.accounts.map((a: Account) => ({
        name: a.name,
        type: a.type,
