...
import { ensureCerts, createConnectHandler } from './mitm.ts';
import { patchAccountUuid } from './account-uuid-rewrite.ts';
import { sanitizeToolPairs } from './tool-pair-sanitize.ts';
import { TopLevelFieldFinder, modelGlobMatches, parseRequestModel, parseAdvisorModel } from './model.ts';
import { BodyWriter } from './request-log.ts';
import { upstreamFetch } from './upstream-fetch.ts';
import type { FakeResponse } from './upstream-fetch.ts';
import { sxTunnelAgent } from './sx.ts';
import type { SxManager } from './sx.ts';
import { createEgressGuard } from './egress-guard.ts';
import type { EgressGuard } from './egress-guard.ts';
import { principalStillAuthorized, resolvePrincipal } from './client-auth.ts';
import type { Config, Dict, Principal, RequestInfo, ServerHooks } from './types.ts';
import type { FakeHeaders } from './upstream-fetch.ts';
...
const LOCAL_OPERATOR: Principal = { role: 'operator', clientId: 'local', clientName: 'Local operator', local: true };
...
const isBodyless = (method: string | undefined): boolean => BODYLESS_METHODS.has(method ?? '');
...
function sendJson(res: http.ServerResponse, status: number, body: Dict): void {
  res.writeHead(status, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify(body));
}

function sendNoStoreJson(res: http.ServerResponse, status: number, body: Dict): void {
  res.writeHead(status, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
  res.end(JSON.stringify(body));
}

function apiError(type: string, message: string): Dict {
  return { type: 'error', error: { type, message } };
}

function sendRateLimited(res: http.ServerResponse, retryAfterSeconds: number, message: string): void {
...
export function safeKeyEqual(a: unknown, b: unknown): boolean {
...
export function isLoopbackAddr(addr: string | null | undefined): boolean {
  return addr === '127.0.0.1' || addr === '::1' || addr === '::ffff:127.0.0.1';
}

// A local process reaching a Tailscale-only bind; a remote peer cannot forge the server's own source address.
export function isSelfConnection(remoteAddress: unknown, localAddress: unknown): boolean {
...
export function createProxyServer(accountManager: any, config: Config, { hooks = {}, sx = null }: ProxyServerOptions = {}) {
  ...
  const requestHandler = async (req: http.IncomingMessage, res: http.ServerResponse) => {
    try {
      ...
      const principal = resolvePrincipal(config, (req.headers['x-api-key'] as string | undefined) ?? undefined, { local: isLocal });
      if (!principal) {
        sendJson(res, 401, apiError('authentication_error', 'Invalid proxy API key'));
        return;
      }
      (req as Dict).jaynsharePrincipal = principal;
  ...
  let certsPromise: Promise<{ caPath: string; caCertPem: string; leafCertPem: string; leafKeyPem: string }> | null = null;
  ...
  server.on('connect', createConnectHandler({ config, accountManager, ensureLeaf, logDir, hooks, log: console.error, sx, egress }) as any);
  server.on('upgrade', createUpgradeRelay({ upstream, sx }) as any);
...
function serveUsage(requestUrl: URL, res: http.ServerResponse, { accountManager, hooks, principal }: Control): void {
  const suppliedSessionIds = requestUrl.searchParams.getAll('session_id');
  let session: { account: string; lastRoutedAt: string } | null | undefined;
...
function serveAccountSelection(requestUrl: URL, res: http.ServerResponse, { accountManager }: Control): void {
...
function serveStatus(res: http.ServerResponse, { accountManager, hooks }: Control): void {
  const status = accountManager.getStatus();
  const extra = hooks.getStatusExtra?.() || {};
...
async function serveReload(res: http.ServerResponse, { hooks }: Control): Promise<void> {
  if (!hooks.reload) {
...
async function serveSwitch(req: http.IncomingMessage, res: http.ServerResponse, { accountManager }: Control): Promise<void> {
  const names = () => (accountManager.accounts || []).map((a: Dict) => a.name);
  let target: unknown;
  try {
...
export function publicUsageSnapshot(status: Dict = {}, extra: Dict = {}, session: unknown = undefined): Dict {
  ...
  return {
    ...
    accounts: (status.accounts || []).map((account: Dict) => ({
      name: account.name,
      type: account.type,
...
export function isSameOriginControlRequest(req: http.IncomingMessage): boolean {
  const site = req.headers['sec-fetch-site'];
  if (site) return site === 'same-origin' || site === 'none';
  return !req.headers.origin; // any Origin on a local control POST means a page issued it
}

async function readControlBody(req: http.IncomingMessage, limit = 64 * 1024): Promise<string> {
  const chunks: Buffer[] = [];
  let size = 0;
...
export function resolveAccountPin(accountManager: any, token: unknown): number | null {
  const accounts: Dict[] = accountManager.accounts || [];
  const norm = (s: unknown) => String(s ?? '').trim().toLowerCase();
  const normalizedToken = norm(token);
  if (!normalizedToken) return null;

  const matchingIndex = (pick: (a: Dict) => unknown) => accounts.findIndex(a => norm(pick(a)) === normalizedToken);
...
function relayHttpForward(req: http.IncomingMessage, res: http.ServerResponse): void {
  let target: URL;
...
function sxAgent(sx: SxManager, targetHost: string): http.Agent {
  return sxTunnelAgent(sx, targetHost);
}

function sxAgentFor({ sx }: { sx?: SxManager | null }, targetHost: string): http.Agent | undefined {
  const useProxy = !!(sx?.useByDefault() && sx.isProvisioned());
  return useProxy ? sxAgent(sx as SxManager, targetHost) : undefined;
}

type HeaderSkipFn = (lowerKey: string) => boolean;

/** Client request headers minus h2 pseudo-headers and whatever `skip` names. */
function copyClientHeaders(headers: http.IncomingHttpHeaders, skip: HeaderSkipFn): http.OutgoingHttpHeaders {
...
function copyResponseHeaders(headers: Dict, { decompressed = true }: { decompressed?: boolean } = {}): Dict {
  const out: Dict = {};
  for (const [key, value] of Object.entries(headers)) {
...
function clientResponseHeaders(entries: [string, string | string[] | undefined][]): Dict {
...
function relayStream(req: http.IncomingMessage, res: http.ServerResponse, target: { upstream: string; sx?: SxManager | null }): void {
...
export interface UpgradeRelayTarget {
  upstream: string;
  sx?: SxManager | null;
}

/** An 'upgrade' listener: relays the WebSocket handshake with the client's own headers, then splices the two sockets. */
export function createUpgradeRelay(target: UpgradeRelayTarget): (req: http.IncomingMessage, socket: import('node:stream').Duplex, head: Buffer) => void {
...
async function relayRaw(req: http.IncomingMessage, res: http.ServerResponse, { upstream, sx }: { upstream: string; sx?: SxManager | null }): Promise<void> {
  ...
  try {
    ...
    res.writeHead(upstreamRes.status!, copyResponseHeaders(upstreamRes.headers as Dict));
    res.end(responseBody);
  } catch (err) {
    console.error('[Jaynshare] Raw relay error:', (err as Error).message);
    if (!res.headersSent) sendJson(res, 502, apiError('proxy_error', 'Upstream unreachable'));
  }
}
...
export interface ProxyListenerOptions {
  ...
  holdMs?: number;
  config?: Config;
  forcedPin?: string | null;
  preferredAccount?: string | null;
  forcedPrincipal?: Principal | null;
  egress?: EgressGuard | null;
}
...
export function createProxyRequestListener({ accountManager, upstream, logDir = null, hooks = {}, sx = null, holdMs = 0, config = {} as Config, forcedPin = null, preferredAccount = null, forcedPrincipal = null, egress = null }: ProxyListenerOptions) {
  const listenerContext = { accountManager, upstream, hooks, logDir, sx, holdMs, config };
  return async (req: http.IncomingMessage, res: http.ServerResponse) => {
    try {
      const principal = forcedPrincipal || (req as Dict).jaynsharePrincipal || LOCAL_OPERATOR;
...
function eventLogPolicy(req: http.IncomingMessage, config: Config): string {
...
const eventLogBlocked = (req: http.IncomingMessage, config: Config) => eventLogPolicy(req, config) === 'block';
const eventLogHidden = (req: http.IncomingMessage, config: Config) => eventLogPolicy(req, config) !== 'show';

/** The client's own OAuth refresh and credential calls carry their own auth: no account is spent. */
async function relayedVerbatim(req: http.IncomingMessage, res: http.ServerResponse, { upstream, sx }: { upstream: string; sx?: SxManager | null }): Promise<boolean> {
 ...
 */
function resolvePins(clientExchange: ClientExchange, { accountManager, forcedPin, preferredAccount }:
  { accountManager: any; forcedPin: string | null; preferredAccount: string | null }): Pins | null {
...
interface ClientExchange {
  ...
  activity: ServerHooks;
}

/** Reads the request, then relays it through the pool and reports what happened. */
async function serveExchange({ req, res, activity }: ClientExchange, { listenerContext, principal, pins }:
  { listenerContext: Dict; principal: Principal; pins: Pins }): Promise<void> {
  const { accountManager, upstream, hooks, logDir, sx, holdMs, config } = listenerContext;
  const reqId = ++requestCounter;
  const sessionId = (req.headers['x-claude-code-session-id'] as string | undefined) || null;
  ...
  const { body, model } = await readBody(req, (found: string) => activity.onRequestModel?.(reqId, { model: found }));
  const advisorModel = parseAdvisorModel(body);

  const blockedBy = model ? (config?.blockedModels || []).find((p: string) => modelGlobMatches(p, model)) : null;
  if (blockedBy) {
  ...
  const ctx: Dict = { account: null, status: null, tried: new Set<number>(), reauthed: new Set<number>(), model, advisorModel, ...pins, holdBudgetMs: holdMs, sessionId: sessionKey, publicSessionId: sessionId, ...client, retryCount: 0, rotated: false };
  const exchange: Dict = { req, res, body, ctx, accountManager, upstream, hooks, reqId, logDir, sx };
...
async function egressPinned(egress: EgressGuard, res: http.ServerResponse): Promise<boolean> {
...
function urlPinToken(url: string): { token: string; rest: string } | null {
...
function rejectUnknownAccount({ req, res, activity }: ClientExchange, label: string, message: string): void {
  const reqId = ++requestCounter;
  const sessionId = (req.headers['x-claude-code-session-id'] as string | undefined) || null;
...
async function readBody(req: http.IncomingMessage, onModel: (model: string) => void): Promise<{ body: Buffer; model: string | null }> {
  const bodyChunks: Buffer[] = [];
  const modelFinder = new TopLevelFieldFinder('model');
  for await (const chunk of req) {
    bodyChunks.push(chunk as Buffer);
    if (!modelFinder.done) {
      const found = modelFinder.push(chunk as Buffer);
...
interface RequestLog {
  write(s: string): void;
  body(label: string, buf: Buffer, contentType?: string): void;
  bodyWriter(label: string, contentType: string): BodyWriter;
  end(): void;
}

// Streams to disk as bytes flow, so a huge response costs only the current chunk.
function openRequestLog(logDir: string, reqId: number): RequestLog {
  ...
  let ended = false;
  const write = (s: string) => { if (!ended && s) ws.write(Buffer.from(String(s), 'latin1')); };
  return {
    write,
    body(label: string, buf: Buffer, contentType?: string) {
      if (!buf || !buf.length) { write(`\n\n=== ${label} ===\n(empty)`); return; }
      new BodyWriter(write, label, contentType || '').chunk(buf);
    },
    bodyWriter(label: string, contentType: string) { return new BodyWriter(write, label, contentType || ''); },
    end() { if (!ended) { ended = true; ws.end('\n'); } },
  };
}

function formatHeaders(headers: Dict | FakeHeaders): string {
  if ((headers as FakeHeaders).entries) {
    return [...(headers as FakeHeaders).entries()].map(([k, v]) => `  ${k}: ${v}`).join('\n');
  }
  return Object.entries(headers).map(([k, v]) => `  ${k}: ${v}`).join('\n');
}

function errorClassForStatus(status: number | null | undefined): string | null {
 ...
 */
async function forwardRequest(exchange: Dict, retryCount: number, useSx?: boolean): Promise<void> {
  ...
  const route: boolean = useSx === undefined ? !!(sx?.useByDefault()) : useSx;

  const account = pickAccount({ accountManager, ctx });
  if (!account) {
  ...
  const attempt: Dict = { account, retryCount, route };
...
function pickAccount({ accountManager, ctx }: { accountManager: any; ctx: Dict }): Dict | null {
...
async function respondNoAccount(exchange: Dict, retryCount: number, route: boolean): Promise<void> {
  ...
  const rejected: Set<string> | undefined = ctx.credentialRejected;
...
function upstreamHeaders(reqHeaders: http.IncomingHttpHeaders, account: Dict): Record<string, any> {
...
function outboundBody(body: Buffer, req: http.IncomingMessage, account: Dict): Buffer {
...
function lazyRequestLog({ req, body, reqId, logDir }: Dict, attempt: Dict): { get(): RequestLog | null; head(): void } {
  ...
  return {
    ...
    head() {
      ...
      headWritten = true;
      const safeHeaders: Dict = { ...attempt.headers };
...
async function sendUpstream({ req, res, accountManager, sx }: Dict, { account, headers, url, body, route }: Dict): Promise<FakeResponse | null> {
...
function rateLimitHeadersOf(upstreamRes: FakeResponse): Dict {
  const rateLimitHeaders: Dict = {};
...
async function handle429(exchange: Dict, attempt: Dict, upstreamRes: FakeResponse): Promise<void> {
...
function rejectedBucket(rateLimitHeaders: Dict): 'general' | 'fable' | null {
...
function switchAfterQuotaRejection(exchange: Dict, attempt: Dict, { bucket, retryAfter }: { bucket: string; retryAfter: number }): void {
  const { res, accountManager, ctx } = exchange;
  ...
  ctx.tried.add(account.index);
  if (res.destroyed) return;
  void forwardRequest(exchange, retryCount + 1, route);
}

/** A burst, not exhaustion: a fresh egress IP or a short wait beats rotating off a warm cache. */
async function absorbRateLimit(exchange: Dict, attempt: Dict, retryAfter: number): Promise<void> {
...
async function handle403(exchange: Dict, { account, retryCount, route }: Dict, upstreamRes: FakeResponse): Promise<void> {
  await upstreamRes.body?.cancel();
...
function canForceRefresh({ accountManager, ctx }: Dict, { account, retryCount }: Dict): boolean {
  return account.type === 'oauth' && !!account.refreshToken
    && retryCount < accountManager.accounts.length && !ctx.reauthed.has(account.index);
}

async function handle401(exchange: Dict, { account, retryCount, route }: Dict, upstreamRes: FakeResponse): Promise<void> {
...
async function relayResponse({ res, accountManager, ctx }: Dict, { account, log }: Dict, upstreamRes: FakeResponse): Promise<void> {
  log.head();
...
function isTransientUpstreamError(err: unknown): boolean {
  ...
  return err instanceof Error &&
    (e.code === 'JAYNSHARE_HEADERS_TIMEOUT' || e.code === 'JAYNSHARE_BODY_TIMEOUT' ||
      e.name === 'TimeoutError' || e.name === 'AbortError' ||
      e.message.includes('fetch failed') ||
      e.code === 'ECONNRESET' || e.code === 'ECONNREFUSED' ||
      e.code === 'ETIMEDOUT' || e.code === 'UND_ERR_CONNECT_TIMEOUT' ||
      e.code === 'UND_ERR_HEADERS_TIMEOUT' || e.code === 'UND_ERR_BODY_TIMEOUT');
}

async function handleUpstreamError(exchange: Dict, { account, retryCount, route, log }: Dict, err: unknown): Promise<void> {
  const { res, accountManager, ctx } = exchange;
  console.error(`[Jaynshare] Upstream error (account "${account.name}"):`, (err as Error).message);

  log.head();
  const requestLog = log.get();
  if (requestLog) { requestLog.write(`\n\n=== ERROR ===\n${(err as Error).stack || (err as Error).message}`); requestLog.end(); }
  ...
  if (!res.headersSent) {
    sendJson(res, 502, apiError('proxy_error', `Upstream error: ${(err as Error).message}`));
...
function resolveBodyIdleTimeout(): number {
...
export function readWithIdleTimeout<T>(reader: ReadableStreamDefaultReader<T>, ms: number): Promise<ReadResult<T>> {
  let timer: NodeJS.Timeout;
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      const err = new Error(`upstream stream idle for ${ms}ms`) as Error & { code: string };
  ...
  return Promise.race([read, timeout]).then((r) => r as ReadResult<T>).finally(() => clearTimeout(timer));
}

async function streamResponse(webStream: ReadableStream, res: http.ServerResponse, { accountManager, accountIndex, bodyWriter }:
  { accountManager: any; accountIndex: number; bodyWriter: BodyWriter | null }): Promise<void> {
  ...
  try {
    while (true) {
      ...
      sseBuffer += decoder.decode(value as Uint8Array, { stream: true });
...
function parseSSEUsage(event: string, accountIndex: number, accountManager: any): void {
...
function extractUsageFromBody(buffer: Buffer, accountIndex: number, accountManager: any): void {
  try {
...
export function rewriteModel(body: Buffer, modelMap: Record<string, string>): Buffer {
  try {
...
function computeRetryAfter(accounts: Dict[]): number {
