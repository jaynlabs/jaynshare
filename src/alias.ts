...
export function jaynshareRef(): string {
...
export function aliasLine(shell: string = detectShell(), ref: string = jaynshareRef()): string {
  const body = `${ref} run --`;
  if (shell === 'fish') return `alias claude '${body}'`;
  return `alias claude='${body}'`;
}

export function rcPathForShell(shell: string = detectShell()): string {
  const home = homedir();
  switch (shell) {
...
export function printAlias({ shell = detectShell() }: { shell?: string } = {}): void {
...
export function installAlias({ shell = detectShell(), rcPath = rcPathForShell(shell) }: { shell?: string; rcPath?: string } = {}): void {
  const line = aliasLine(shell);
...
export function uninstallAlias({ shell = detectShell(), rcPath = rcPathForShell(shell) }: { shell?: string; rcPath?: string } = {}): void {
