...
import { encodePinComponent } from './claude-env.ts';
...
export class Warmer extends IntervalJob {
  readonly label = 'Keep-warm';
  declare accountManager: { accounts: Account[]; ensureTokenFresh(i: number): Promise<void> };
  port: number;
  apiKey: string | null;
  model: string;
  prompt: string;
  spawnFn: (spec: SpawnSpec) => Promise<number>;
  timeoutMs: number;
  _abort: AbortController | null = null; // AbortController of the in-flight sweep
  ...
  constructor(accountManager: any, {
    ...
    intervalMs?: number; port: number; apiKey?: string | null; model?: string; prompt?: string;
    spawnFn?: (spec: SpawnSpec) => Promise<number>; timeoutMs?: number; log?: (msg: string) => void;
  }) {
  ...
  _isWarmTarget(account: Account): boolean {
  ...
  async sweep(): Promise<void> {
    ...
    try {
      const targets = this.accountManager.accounts.filter((account: Account) => this._isWarmTarget(account));
  ...
  async warmAccount(account: Account, signal: AbortSignal): Promise<void> {
  ...
  _spawnSpec(account: Account, signal: AbortSignal): SpawnSpec {
  ...
  getStatus() {
    return {
      ...
      accounts: this._accountStatusRows(
        (account: Account) => account.type === 'oauth' && !account.upstream,
        'lastWarmedAt',
      ),
    };
  }
...
function defaultSpawn({ command, args, env, timeoutMs, signal }: SpawnSpec): Promise<number> {
