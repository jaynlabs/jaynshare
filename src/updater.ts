...
export function compareVersions(a: string, b: string): number {
  const numericParts = (v: string) => String(v).split('+')[0]!.split('-')[0]!.split('.').map((n) => parseInt(n, 10) || 0);
...
export function installKind({ root = packageRoot(), globalRoot = npmGlobalRoot }:
  { root?: string; globalRoot?: (() => string | null) | string | null } = {}): 'git' | 'global' | 'local' | 'unknown' {
...
export async function fetchLatestVersion({ fetchImpl = fetch, timeoutMs = 5000 }:
  { fetchImpl?: typeof fetch; timeoutMs?: number } = {}): Promise<string | null> {
...
interface UpdateCache { checkedAt?: number; latest?: string | null; }
...
export async function checkForUpdate({
  ...
  current?: string | null; cachePath?: string; fetchImpl?: typeof fetch; now?: number; intervalMs?: number; force?: boolean;
...
export function runUpdate(version = 'latest', { spawnImpl = spawnSync }: { spawnImpl?: typeof spawnSync } = {}): boolean {
...
export async function autoUpdate({ config = {}, force = false, log = console.error }:
  { config?: any; force?: boolean; log?: (msg: string) => void } = {}): Promise<Record<string, any>> {
