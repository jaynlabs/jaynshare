...
import { proxyForHost, proxyAgent } from './upstream-proxy.ts';
...
function resolveHeadersTimeout(perCall: number | null | undefined): number {
...
function headersTimeoutError(ms: number): Error & { code: string } {
  const err = new Error(`upstream response headers timed out after ${ms}ms`) as Error & { code: string };
...
export function upstreamFetch(url: string, opts: UpstreamFetchOptions = {}, via: SxManager | null = null): Promise<FakeResponse> {
...
export function proxyFetch(url: string, opts: UpstreamFetchOptions = {}): Promise<FakeResponse> {
  const { headersTimeoutMs, ...rest } = opts;
  if (!proxyForHost(new URL(url).hostname)) return fetch(url, rest as RequestInit) as unknown as Promise<FakeResponse>;
  return pooledFetch(url, { ...rest, headersTimeoutMs: resolveHeadersTimeout(headersTimeoutMs) });
}

function pooledFetch(url: string, opts: UpstreamFetchOptions): Promise<FakeResponse> {
  ...
  if (proxy) {
    ...
    return nodeRequest(parsedUrl, opts, { transport: isHttp ? (http as HttpLikeTransport) : (https as unknown as HttpLikeTransport), agent });
  }
  return nodeRequest(parsedUrl, opts, { transport: isHttp ? (http as HttpLikeTransport) : (https as unknown as HttpLikeTransport), agent: isHttp ? httpAgent : httpsAgent });
}
...
function directFetch(url: string, { headersTimeoutMs, ...opts }: UpstreamFetchOptions): Promise<FakeResponse> {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(headersTimeoutError(headersTimeoutMs!)), headersTimeoutMs!);
  timer.unref?.();
  return fetch(url, { ...opts, signal: ctrl.signal } as RequestInit).then(
    (res) => { clearTimeout(timer); return res as unknown as FakeResponse; },
    (err) => { clearTimeout(timer); throw err; },
  );
}

function proxiedFetch(url: string, opts: UpstreamFetchOptions, sx: SxManager): Promise<FakeResponse> {
  const parsedUrl = new URL(url);
...
type HttpLikeTransport = { request(url: URL, options: http.RequestOptions, callback?: (res: http.IncomingMessage) => void): http.ClientRequest };

function nodeRequest(parsedUrl: URL, opts: UpstreamFetchOptions, { transport, agent }: { transport: HttpLikeTransport; agent: http.Agent }): Promise<FakeResponse> {
...
function nodeToWeb(res: http.IncomingMessage): ReadableStream {
  let closed = false;
...
function makeResponse(res: http.IncomingMessage): FakeResponse {
  ...
  return {
    ...
    async arrayBuffer() { const b = await collect(); return b.buffer.slice(b.byteOffset, b.byteOffset + b.byteLength) as ArrayBuffer; },
  };
}
...
function makeHeaders(h: Record<string, string | string[] | undefined>): FakeHeaders {
  const flat = (v: string | string[] | undefined) => (Array.isArray(v) ? v.join(', ') : (v ?? null));
  const entries = function* (): IterableIterator<[string, string]> {
    for (const [k, v] of Object.entries(h)) yield [k, flat(v) as string];
  };
  return {
    get: (name: string) => { const v = h[name.toLowerCase()]; return v == null ? null : flat(v); },
    entries,
