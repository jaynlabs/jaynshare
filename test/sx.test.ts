...
import { connectThroughProxy, tunnelTls, SxManager } from '../src/sx.ts';
import { upstreamFetch } from '../src/upstream-fetch.ts';

const T = { timeout: 30000 };
const listen = (s) => new Promise<number>((r) => s.listen(0, '127.0.0.1', () => r((s.address() as { port: number }).port)));
...
test('upstreamFetch goes direct when no sx manager routes the attempt', T, async () => {
  ...
  try {
    ...
    const sx = { isProvisioned: () => false, getProxy: () => ({ host: '127.0.0.1', port: 1 }) } as SxManager;
