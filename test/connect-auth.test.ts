...
import { connectPrincipal } from '../src/mitm.ts';
import { safeKeyEqual, isLoopbackAddr, isSelfConnection } from '../src/server.ts';

// Without the CONNECT gate, a remote client gets a token injected or an open relay.

const sock = (remoteAddress) => ({ remoteAddress }) as Socket;
const req = (auth = null) => ({ headers: auth ? { 'proxy-authorization': auth } : {} }) as IncomingMessage;
const keyed = { proxy: { apiKey: 'secret' } } as Config;

test('no proxy credential configured → CONNECT is open (matches the HTTP path)', () => {
  assert.ok(connectPrincipal(req(), sock('203.0.113.9'), { proxy: {} } as Config));
});
