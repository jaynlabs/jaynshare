...
import { SessionTracker, SESSION_KNOWN_TTL_MS, SESSION_ACTIVE_TTL_MS } from '../src/session-tracker.ts';
...
test('lookup returns a copy without refreshing expiry and drops expired sessions', () => {
  const { clock, now } = fixedClock();
  const st = new SessionTracker({ now });
  st.touch('s1', 2, clock.t);
  assert.deepEqual(st.lookup('s1', clock.t), { accountIndex: 2, lastSeen: clock.t });
  clock.t += SESSION_KNOWN_TTL_MS + 1;
  assert.equal(st.lookup('s1', clock.t), null);
