...
function makeTUI() {
  ...
  const config = { proxy: { port: 1 }, accounts: [{ name: 'a', type: 'oauth' }], routes: [], blockedModels: [] } as any;
  return new TUI({
...
test('the tick is slow while idle and fast only while something is animating', () => {
  ...
  tui.active.set(1, { started: Date.now() });
...
test('a request arriving while idle re-arms the tick at once', () => {
  ...
  tui.onRequestStart(1, { method: 'POST', path: '/v1/messages' });
  ...
  tui.onRequestStart(2, { method: 'POST', path: '/v1/messages' });
