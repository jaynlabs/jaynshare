...
import { buildClaudeEnvLines } from '../src/claude-env.ts';

test('MITM mode (default) emits proxy vars + CA cert, and clears ANTHROPIC_BASE_URL', () => {
  const lines = buildClaudeEnvLines({ port: 3456, caPath: '/home/u/.config/jaynshare-ca.pem' });
  assert.deepEqual(lines, [
    ...
    'export no_proxy=localhost,127.0.0.1,::1',
    'export NODE_EXTRA_CA_CERTS=/home/u/.config/jaynshare-ca.pem',
...
test('an account pin rides in the proxy userinfo under MITM', () => {
  ...
  assert.ok(lines.includes('unset JAYNSHARE_ACCOUNT'));
});

test('an account pin becomes a /jaynshare-account/ prefix under --no-mitm', () => {
  const lines = buildClaudeEnvLines({ port: 8080, useMitm: false, account: 'work (Acme)' });
  assert.deepEqual(lines, [
    'export ANTHROPIC_BASE_URL=http://localhost:8080/jaynshare-account/work%20%28Acme%29',
    'unset JAYNSHARE_ACCOUNT',
  ]);
});
