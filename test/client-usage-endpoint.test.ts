...
import { hashClientSecret } from '../src/client-auth.ts';
import { createProxyServer, publicUsageSnapshot } from '../src/server.ts';
...
function remoteRequest(server, url, key = null) {
  const req = Object.assign(Readable.from([]), {
    ...
    socket: { remoteAddress: '203.0.113.9', localAddress: '100.64.0.1' },
  });
  ...
  const complete = new Promise<void>(resolve => { res.done = resolve; });
