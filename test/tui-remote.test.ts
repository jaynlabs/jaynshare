...
import { RemoteControl, createAttachSession } from '../src/tui-remote.ts';
import { asConfig } from '../test-support/test-helpers.ts';
...
const logged = (tui: { log: unknown[] }) => waitFor(() => tui.log.length > 0, 'a message in the pane');
const reached = (seen: unknown[]) => waitFor(() => seen.length > 0, 'a request to reach the server');
// Only for asserting that nothing happens.
const nothingHappens = () => new Promise(r => setTimeout(r, 50));

const HOUR = 3600_000;

function statusFixture(over: Dict = {}): Dict {
...
async function fakeServer(t: { after(fn: () => unknown): void }, routes: Record<string, HttpHandler> = {}) {
  const seen = [];
  ...
  t.after(() => new Promise<any>(resolve => { server.closeAllConnections(); server.close(resolve); }));
  return { server, seen, port: (server.address() as { port: number }).port };
}

// The three members _call reads off a fetch reply.
const reply = (status: number, body: string): Response => ({ ok: status >= 200 && status < 300, status, text: async () => body } as Response);

const json = (payload: Dict): HttpHandler => (req, res) => {
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify(payload));
};

async function makeSession(t: { after(fn: () => unknown): void }, { routes, status = statusFixture() }: { routes?: Record<string, HttpHandler>; status?: Dict } = {}) {
  ...
  const session = createAttachSession({
    control,
    config: { accounts: [], proxy: { port: fake.port, apiKey: 'secret' } },
...
test('a poll that never gets an answer becomes a lost connection, not a live view', async (t) => {
  ...
  const session = createAttachSession({ control, config: { accounts: [], proxy: { port: fake.port } }, onQuit: () => {} });
  session.tui.render = () => {};
...
test('the poll deadline is derived from the poll interval', async (t) => {
  ...
  const session = createAttachSession({ control, config: { accounts: [], proxy: { port: fake.port } }, onQuit: () => {}, pollMs: 4000 });
  assert.equal(control.timeoutMs, 12_000);
  session.stop();

  const fixed = new RemoteControl({ port: fake.port, timeoutMs: 250 });
  createAttachSession({ control: fixed, config: asConfig(), onQuit: () => {}, pollMs: 4000 }).stop();
