...
import type { Dict, ServerHooks } from '../src/types.ts';
...
const CONFIG = asConfig({ proxy: { apiKey: 'tc-test' }, upstream: 'https://api.anthropic.com' });
const ACCTS = [
...
async function post(port: number, body: string): Promise<{ status: number; body: Dict }> {
  const res = await fetch(`http://127.0.0.1:${port}/jaynshare/switch`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body,
  });
  return { status: res.status, body: (await res.json()) as Dict };
}

async function withServer(
  fn: (am: AccountManager, port: number, server: Server) => Promise<void>,
  hooks: ServerHooks = {},
) {
...
test('the status endpoint reports the switched account', async () => {
  await withServer(async (am, port) => {
    ...
    const status = (await res.json()) as Dict;
...
function remoteRequest(server: Server, { headers = {}, body = '{}' }: { headers?: Dict; body?: string } = {}) {
  const req = Object.assign(Readable.from([Buffer.from(body)]), {
    method: 'POST',
    url: '/jaynshare/switch',
    headers,
  ...
  const res: Dict = {
    ...
    end(chunk?: string) { if (chunk) this.chunks += chunk; this._done(); },
  };
  const finished = new Promise<void>(resolve => { res._done = resolve; });
  server.emit('request', req, res);
  return finished.then(() => ({ status: res.status as number, body: JSON.parse(String(res.chunks) || '{}') as Dict }));
}
