...
test('installKind flags a global npm install and distinguishes a local one', () => {
  ...
  const global = `${gRoot}/@jaynlabs/jaynshare`;
  const local = '/home/x/project/node_modules/@jaynlabs/jaynshare';
...
test('installKind is unknown outside node_modules (e.g. running from source path)', () => {
  assert.equal(installKind({ root: '/opt/jaynshare-src', globalRoot: () => null }), 'unknown');
});

// ── fetchLatestVersion ───────────────────────────────────────

test('fetchLatestVersion reads dist-tags.latest from the registry', async () => {
  const fetchImpl: any = async () => ({ ok: true, json: async () => ({ 'dist-tags': { latest: '1.2.3' } }) });
...
test('fetchLatestVersion returns null on a non-ok response or a throw', async () => {
  assert.equal(await fetchLatestVersion({ fetchImpl: async () => ({ ok: false }) as any }), null);
...
test('checkForUpdate fetches when uncached and reports an available update', async () => {
  ...
  let calls = 0;
  const fetchImpl: any = async () => { calls++; return { ok: true, json: async () => ({ 'dist-tags': { latest: '2.0.0' } }) }; };
...
test('checkForUpdate refetches once the interval elapses, and force overrides', async () => {
  const cachePath = tmpCache();
  let calls = 0;
  const fetchImpl: any = async () => { calls++; return { ok: true, json: async () => ({ 'dist-tags': { latest: '1.0.0' } }) }; };
...
test('checkForUpdate reports no update when already on the latest', async () => {
  const cachePath = tmpCache();
  const fetchImpl: any = async () => ({ ok: true, json: async () => ({ 'dist-tags': { latest: '1.1.1' } }) });
...
test('runUpdate invokes the global npm install for the requested version', () => {
  const calls: unknown[] = [];
  const spawnImpl: any = (cmd: string, argv: string[]) => { calls.push([cmd, argv]); return { status: 0 }; };
...
test('runUpdate returns false when npm fails', () => {
  assert.equal(runUpdate('2.3.4', { spawnImpl: (() => ({ status: 1 })) as any }), false);
  assert.equal(runUpdate('2.3.4', { spawnImpl: (() => ({ error: new Error('ENOENT') })) as any }), false);
});
