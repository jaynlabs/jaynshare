...
import { Warmer } from '../src/warmer.ts';
...
function fakeSpawner(result: number | Error = 0) {
  const calls = [];
  const fn = async (spec) => {
    calls.push(spec);
...
test('warms only healthy, idle Anthropic OAuth accounts with no live 5h window', async () => {
  ...
  assert.ok(spawn.calls[0].env.ANTHROPIC_BASE_URL.endsWith('/jaynshare-account/idle'), spawn.calls[0].env.ANTHROPIC_BASE_URL);
});
...
test('the spawn invocation is a minimal non-interactive claude pinned to the account', async () => {
  ...
  assert.equal(spec.env.ANTHROPIC_BASE_URL, 'http://127.0.0.1:9999/jaynshare-account/solo');
