...
import { generateCertChain } from '../src/x509.ts';
import { createConnectHandler, resolveConnectPin, connectPinToken } from '../src/mitm.ts';
import { AccountManager } from '../src/account-manager.ts';
import { ACCOUNT_PREFERENCE_PREFIX, encodeAccountPreference } from '../src/account-preference.ts';

// In MITM mode the pin arrives as the Basic username of Proxy-Authorization on the CONNECT.

function listen(server: any): Promise<number> { return new Promise(r => server.listen(0, '127.0.0.1', () => r((server.address() as { port: number }).port))); }
...
function connectThroughProxy(proxyPort: number, target: string, caCertPem: string, alpn: string[], proxyAuth: string | null = null): Promise<tls.TLSSocket> {
...
function makeUpstream(handler: (req: http.IncomingMessage, body: string) => any) {
...
function makeProxy(am: any, upPort: number, { leafCertPem, leafKeyPem }: any, config: any = {}) {
...
async function postOverTunnel(tlsSock, headers = {}) {
  const client = http2.connect('https://localhost', { createConnection: () => tlsSock } as any);
...
test('the Basic username selects the account', () => {
  ...
  assert.deepEqual(resolveConnectPin({ headers: { 'proxy-authorization': basic('work:k') } }, { accountManager: am, config: { accounts: [], proxy: { apiKey: 'k' } } }), { pin: 'work', error: null });
...
test('a username equal to the proxy key is auth, not a pin', () => {
  ...
  assert.deepEqual(resolveConnectPin({ headers: { 'proxy-authorization': basic('secret:') } }, { accountManager: am, config: { accounts: [], proxy: { apiKey: 'secret' } } }), { pin: null, error: null });
  // Even when an account is (unwisely) named after the key, auth wins.
  const clash = { accounts: [{ name: 'secret' }] };
  assert.deepEqual(resolveConnectPin({ headers: { 'proxy-authorization': basic('secret:') } }, { accountManager: clash, config: { accounts: [], proxy: { apiKey: 'secret' } } }), { pin: null, error: null });
});

test('an unknown username is an error rather than an ignored pin', () => {
  const am = { accounts: [{ name: 'work' }] };
  const { pin, error } = resolveConnectPin({ headers: { 'proxy-authorization': basic('typo:') } }, { accountManager: am, config: { accounts: [], proxy: { apiKey: 'secret' } } });
...
test('no header, or a Bearer key, yields no pin', () => {
  const am = { accounts: [{ name: 'work' }] };
  assert.deepEqual(resolveConnectPin({ headers: {} }, { accountManager: am, config: { accounts: [], proxy: { apiKey: 'secret' } } }), { pin: null, error: null });
  assert.deepEqual(resolveConnectPin({ headers: { 'proxy-authorization': 'Bearer secret' } }, { accountManager: am, config: { accounts: [], proxy: { apiKey: 'secret' } } }), { pin: null, error: null });
...
test('a pin on a blind-tunneled host is ignored, not refused', T, async () => {
  ...
  try {
    ...
    const [chunk] = await once(raw, 'data') as [Buffer];
