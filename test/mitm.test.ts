...
const { ensureCerts, caCertPath, TEST_HOST } = await import('../src/mitm.ts');
const { AccountManager } = await import('../src/account-manager.ts');
const { createProxyServer } = await import('../src/server.ts');

function listen(server) {
  return new Promise<number>((r) => server.listen(0, '127.0.0.1', () => r((server.address() as { port: number }).port)));
}
...
test('CONNECT to the test host is intercepted and answered locally (proxy + CA proof)', async () => {
  ...
  const proxy = createProxyServer(am, { accounts: [], proxy: { apiKey: 'k' }, upstream: 'https://api.anthropic.com' }, { hooks: {} });
  ...
  try {
    ...
    assert.match(resp, /"jaynshare":"mitm-proxy-ok"/);
