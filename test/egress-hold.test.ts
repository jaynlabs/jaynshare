...
import { createProxyRequestListener } from '../src/server.ts';
import { EgressGuard } from '../src/egress-guard.ts';
...
test('with the egress pinned and matching, requests pass through untouched', async () => {
  ...
  const guard = new EgressGuard({ pin: '203.0.113.7', fetchImpl: probeReturning('203.0.113.7') as unknown as typeof fetch });
...
test('an unpinned egress holds the request and never reaches upstream', async () => {
  ...
  const guard = new EgressGuard({
    ...
    fetchImpl: probeReturning('192.0.2.55') as unknown as typeof fetch,          // VPN down: home address
  });
  const { proxy, port } = await proxyWith(guard, upstreamPort);
...
test('a request held through a flap is sent once the pinned egress returns', async () => {
  ...
  const guard = new EgressGuard({
    ...
    fetchImpl: probeReturning(['192.0.2.55', '192.0.2.55', '203.0.113.7']) as unknown as typeof fetch,
  });
  const { proxy, port } = await proxyWith(guard, upstreamPort);
