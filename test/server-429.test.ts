...
test('a rate-limit 429 never rotates to another account', async () => {
  ...
  const proxy = createProxyServer(am, { accounts: [], proxy: { apiKey: 'k' }, upstream: `http://127.0.0.1:${upstreamPort}` });
  const proxyPort = await listen(proxy);
...
test('stale over-threshold quota is re-probed, not refused forever', async () => {
  ...
  // Simulate restoring a poisoned snapshot from jaynshare.state.json.
