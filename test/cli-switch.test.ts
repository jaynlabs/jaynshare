...
import type { AccountManager as AccountManagerType } from '../src/account-manager.ts';
...
async function writeConfig(port: number): Promise<string> {
...
function runCli(configPath: string, cliArgs: string[]): Promise<{ code: number | null; stdout: string; stderr: string }> {
...
async function withProxy(fn: (am: AccountManagerType, configPath: string) => Promise<void>) {
  const am = new AccountManager(ACCTS, 0.98);
  const proxy = createProxyServer(am, asConfig({ proxy: { apiKey: 'tc-test' }, upstream: 'https://api.anthropic.com' }), { hooks: {} });
...
async function withDisabled(fn: (am: AccountManagerType, configPath: string) => Promise<void>) {
  const accts = [
    { name: 'alice@example.com', type: 'apikey', apiKey: 'k1' },
