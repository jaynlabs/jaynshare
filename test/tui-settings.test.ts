...
import { fakeAccountManager } from '../test-support/test-helpers.ts';
...
function makeTUI({ sx = null }: { sx?: Dict } = {}) {
  const config = { proxy: { port: 1 }, accounts: [{ name: 'a', type: 'oauth' }], routes: [], blockedModels: [] };
  const tui = new TUI({
    accountManager: fakeAccountManager([{ name: 'a', index: 0, type: 'oauth', credential: 't' }]), config, sx,
...
function renderWithCursorAt(tui: TUI, idx: number) {
  tui.setIdx = idx;
  const lines = [];
