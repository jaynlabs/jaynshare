...
import { createProxyServer, resolveAccountPin } from '../src/server.ts';
...
async function withProxy(run) {
  ...
  const am = new AccountManager([oauth('alpha'), oauth('beta')], 0.98);
  const proxy = createProxyServer(am, { accounts: [],
    proxy: { apiKey: 'k' },
...
test('a /jaynshare-account/<name> request is routed to that exact account, prefix stripped', async () => {
  await withProxy(async ({ proxyPort, seen }) => {
    const res = await post(`http://127.0.0.1:${proxyPort}/jaynshare-account/beta/v1/messages`);
    await res.text();
...
test('pinning by numeric index also works', async () => {
  await withProxy(async ({ proxyPort, seen }) => {
    const res = await post(`http://127.0.0.1:${proxyPort}/jaynshare-account/alpha/v1/messages`);
    await res.text();
    assert.equal(res.status, 200);
...
test('an unknown pin returns 404 and never reaches upstream', async () => {
  await withProxy(async ({ proxyPort, seen }) => {
    const res = await post(`http://127.0.0.1:${proxyPort}/jaynshare-account/ghost/v1/messages`);
