...
type ProxyResponse = { status?: number; headers: http.IncomingHttpHeaders; body: string };

function proxyRequest({ proxyPort, method = 'GET', absoluteUrl, body }: { proxyPort: number; method?: string; absoluteUrl: string; body?: string }): Promise<ProxyResponse> {
...
test('forwards an absolute-form HTTP request to its target host, not to Anthropic', async () => {
  ...
  const proxy = createProxyServer(noRouteManager, { accounts: [], proxy: {}, upstream: 'https://api.anthropic.com' });
