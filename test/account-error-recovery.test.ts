...
import { refreshAccessToken } from '../src/oauth.ts';
...
test('refreshAccessToken attaches the HTTP status to a rejection error', async () => {
  ...
  const endpoint = `http://127.0.0.1:${(srv.address() as { port: number }).port}/token`;
  try {
    await assert.rejects(refreshAccessToken('r', endpoint), (e: any) => {
