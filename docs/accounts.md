...
Adding, naming, and managing the accounts Jaynshare rotates between.
...
```bash
jaynshare remove user@example.com --org Acme
```

## Managing accounts

```bash
jaynshare accounts             # list accounts with tier and token status
jaynshare accounts -v          # also show token expiry times
jaynshare remove <name>        # remove an account (by name or email)
jaynshare disable <name>       # temporarily exclude it from rotation
jaynshare enable <name>        # re-enable it (also clears a stuck error state)
jaynshare priority <name> 1    # rotation preference, lower = preferred
jaynshare priority <name> --first
jaynshare priority <name> --last
```
